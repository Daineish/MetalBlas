...
{
    struct PairIdxVal<T: BinaryFloatingPoint>
    {
        var idx : Int
        var val : T
    }

    func metalIxamax<T: BinaryFloatingPoint>(_ N: Int, _ vecx: [T], _ incx: Int) -> Int
    {
        ...
        guard
            ...
            fatalError("Failed to create buffers for matrices in amax")
        }
        metalIxamax(T(0), N, bufferx, incx, &bufferRes)
    ...
    func metalIxamax<T: BinaryFloatingPoint>(_ type: T, _ N: Int, _ bufferx: MTLBuffer, _ incx: Int, _ bufferRes: inout MTLBuffer)
    {
        ...
        let funcString = T.self == Float.self ? "isamax" : "ihamax"
        guard let pipelineState = pipelineStates[funcString] else {
            fatalError("Failed to get pipeline state for function amax")
        }
        ...
        let tgfuncString = T.self == Float.self ? "sreduceMax" : "hreduceMax"
    ...
    func metalIhamax(_ N: Int, _ bufx: MTLBuffer, _ incx: Int, _ bufres: inout MTLBuffer)
    {
        metalIxamax(Float16(0), N, bufx, incx, &bufres)
    }

    func metalIhamax(_ N: Int, _ vecx: [Float16], _ incx: Int) -> Int
    {
        return metalIxamax(N, vecx, incx)
    }

    func metalIsamax(_ N: Int, _ bufx: MTLBuffer, _ incx: Int, _ bufres: inout MTLBuffer)
    {
        metalIxamax(Float(0), N, bufx, incx, &bufres)
    }

    func metalIsamax(_ N: Int, _ vecx: [Float], _ incx: Int) -> Int
    {
        return metalIxamax(N, vecx, incx)
    }

    func metalIdamax(_ N: Int, _ bufx: MTLBuffer, _ incx: Int, _ bufres: inout MTLBuffer)
    {
        metalIxamax(Double(0), N, bufx, incx, &bufres)
    }

    func metalIdamax(_ N: Int, _ vecx: [Double], _ incx: Int) -> Int
    {
        return metalIxamax(N, vecx, incx)
    }
}
